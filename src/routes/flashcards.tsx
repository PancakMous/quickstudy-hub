...
export const Route = createFileRoute("/flashcards")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Flip through interactive History flashcards covering WWI, WWII, Ancient Egypt, Ancient Rome, and major historical figures." },
      { property: "og:title", content: "Flashcards — QuickStudy" },
      { property: "og:description", content: "Flip through interactive History flashcards covering WWI, WWII, Ancient Egypt, Ancient Rome, and major historical figures." },
    ],
  }),
...
function FlashcardsPage() {
  const { customCards, studiedCardIds } = useStudyStore();
  const cards = useMemo(() => [...HISTORY_FLASHCARDS, ...customCards], [customCards]);
  ...
  const addCard = () => {
    ...
    studyActions.addFlashcard(question, answer);
    setQuestion("");
    setAnswer("");
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 pt-10 pb-14 sm:pt-16">
      <div className="mb-5 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-ink font-display">Flashcards</h1>
          <p className="text-sm text-ink/55">Tap the card to flip · History</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <div className="card-glass rounded-[2rem] p-6 sm:p-8">
          ...
          <p className="mt-3 text-center text-xs font-semibold text-ink/40">
            {studiedCardIds.length} of {cards.length} cards studied
          </p>
        </div>

        {/* Add flashcard */}
        <div className="card-glass rounded-[2rem] p-6">
          <h2 className="text-lg font-bold text-ink font-display">Add a flashcard</h2>
